        ...
        setPotInfo((prevState) => ({
          ...
          TokenInPot: {
            ...
            values: {
              ...
              [results[index].returnValues.tokenName]: results[index].returnValues.amount
            },
            usdvalues: {
              ...prevState.TokenInPot.usdvalues,
              [results[index].returnValues.tokenName]: results[index].returnValues.usdValue
            },
          },
        }));
      }
        ...
        tokensInPot: {
          names: prevState.entries
            ...
            .filter(function (value, index, array) {
              ...
              var determine = prevState.entries
                .filter((entry) => entry.round === prevState.round)
                .map((entry) => entry.token)
                ...
                prevState.entries
                  ...
                  .map((name) =>
                    prevState.entries
                      .filter((entry) => entry.round === prevState.round)
                      .filter((entry) => entry.token === val2)
                      .reduce((total, entry_) => {
                        return parseInt(total.usdvalue ? total.usdvalue : total) + parseInt(entry_.usdvalue);
                      }, 0)
                  )[0]
              console.log("This ___:::", determine)
              return determine
            })
          ,
          ...
          usdvalues: prevState.entries
            ...
            ).sort((val2, val1) => { return val1 - val2 }),
        },
      }));
    } catch (error) {
      console.log(error);
    ...
    <div className="Home">
      <div className="Home_content1">
        ...
        <div>
          ...
          />
          {/* <Wheel /> */}
          {/* <img src='/assets/temp.png' className='' /> */}
        </div>
